...
export const useMetaballAnimation = ({containerRef, canvasRef}: { containerRef: React.RefObject<HTMLElement | null>, canvasRef: React.RefObject<HTMLCanvasElement| null> }) => {
    ...
    useEffect(() => {
        ...
        if (!offOutCtx || !offCtx || !tmpOutCtx || !tmpCtx) return;
        ...
        function draw() {
            if (!canvas || !section || !ctx || !offOutCtx || !tmpOutCtx || !tmpCtx || !offCtx) return;
